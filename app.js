...
var APP_VERSION = 'kmb-v12'; // samakan dgn CACHE 'mar-kmb-v12' di sw.js tiap rilis
...
function queueSubmit() {
  ...
  var op = {
    op_id: uuid(), seq: (_enqSeq++), action: 'submit_work', wo_id: activeWo.id, wo_number: activeWo.wo_number,
...
function queueCancel() {
  ...
  var op = {
    op_id: uuid(), seq: (_enqSeq++), action: 'cancel_wo', wo_id: cancelWoId, wo_number: woNum,
...
function queueOverride() {
  ...
  for (var i = 0; i < sels.length; i++) {
    ...
    seen[mid] = true; team.push({ mechanic_id: mid, percentage: 100 }); // KMBfull-point
  }
  ...
  var op = {
    op_id: uuid(), seq: (_enqSeq++), action: 'save_override', wo_id: activeApproval.id, wo_number: activeApproval.wo_number,
...
function queueApprove(level) {
  ...
  var op = {
    op_id: uuid(), seq: (_enqSeq++), action: action, wo_id: activeApproval.id, wo_number: activeApproval.wo_number,
...
function queueReject() {
  ...
  var op = {
    op_id: uuid(), seq: (_enqSeq++), action: 'reject', wo_id: activeApproval.id, wo_number: activeApproval.wo_number,
